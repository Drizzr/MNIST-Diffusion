...
def load_from_checkpoint(args, forward, dataset, val_dataset, writer, device):
    ...
    model.load_state_dict(torch.load(os.path.join(args.load_dir, "model.pth"), weights_only=True, map_location=device))


    optimizer.load_state_dict(torch.load(os.path.join(args.load_dir, "optimizer.pth"), weights_only=True, map_location=device))
    lr_scheduler.load_state_dict(torch.load(os.path.join(args.load_dir, "lr_scheduler.pth"), weights_only=True, map_location=device))
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, help="where the model will be saved", default="checkpoints/")
