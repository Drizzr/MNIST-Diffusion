...
def load_from_checkpoint(args, forward, dataset, val_dataset, writer):
    ...
    optimizer.load_state_dict(torch.load(os.path.join(args.save_dir, "optimizer.pth"), weights_only=True, map_location=torch.device("cpu")))
    lr_scheduler.load_state_dict(torch.load(os.path.join(args.save_dir, "lr_scheduler.pth"), weights_only=True, map_location=torch.device("cpu")))
